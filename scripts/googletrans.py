...
'''Michael Phan: Using Google Translate for free :)
'''
...
class Googletrans(object):
    '''All googletrans API lives in this class

    You have to first create an instance of Googletrans to use this API
    ...
    .. note:: multi thread worker relys on `futures <https://pypi.python.org/pypi/futures>`_, if it is not avalible, ``googletrans`` will work under single thread mode
    
    :Example:

        >>> import googletrans
        >>> 
        >>> # Create a Googletrans instance first
        ...
        en
        >>> # Get googletrans object dedicated for romanlized translation (romanlization)
        >>> gs_roman = googletrans.Googletrans(WRITING_ROMAN)
    ...
    def translate(self, text, target_language, source_language='auto'):
        ...
        .. note::
         ...
         - Input all source strings at once. Googletrans will batch and fetch concurrently for maximize speed.
        ...
        :Example:
         ...
         >>> gs_roman = Googletrans(WRITING_ROMAN)
         >>> print(gs_roman.translate('Hello', 'zh'))
         Nín hǎo
    ...
    def detect(self, text):
        ...
        .. note::
         ...
         - Input all source strings at once. Googletrans will detect concurrently for maximize speed.
         - `futures <https://pypi.python.org/pypi/futures>`_ is required for best performance.
         - It returns generator on batch input in order to better fit pipeline architecture.
...
def _main(argv):
    ...
    gs = Googletrans(writing=writing)
    import fileinput
